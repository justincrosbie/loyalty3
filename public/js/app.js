<<<<<<< HEAD
window.app = angular.module('loyalty3', ['ngCookies', 'ngResource', 'ui.bootstrap', 'ui.select2', 'ngTable', 
										'$strap.directives', 'ngRoute', 'ngAnimate', 'ui.calendar', 'ui.map', 
										'ngff.controllers', 'ngff.directives', 'ngff.services', 'loadingOnAJAX']);

angular.module("template/pagination/pagination.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/pagination/pagination.html",
    "<ul class=\"pagination\">\n" +
    "  <li ng-repeat=\"page in pages\" ng-class=\"{active: page.active, disabled: page.disabled}\"><a ng-click=\"selectPage(page.number)\">{{page.text}}</a></li>\n" +
    "</ul>\n" +
    "");
}]);

// bundling dependencies
window.angular.module('ngff.controllers', 
										[
											'ngff.controllers.header', 
											'ngff.controllers.index', 
                                            'ngff.controllers.bookingscalendar', 
                                            'ngff.controllers.bookingsmap', 
											'ngff.controllers.customers', 
											'ngff.controllers.sites', 
											'ngff.controllers.datasourcetypes', 
											'ngff.controllers.datasources', 
											'ngff.controllers.subscriptions', 
											'ngff.controllers.titles', 
											'ngff.controllers.countrys', 
											'ngff.controllers.currencys', 
											'ngff.controllers.bookingstatuss', 
											'ngff.controllers.bookingtypes', 
											'ngff.controllers.ratecodes', 
											'ngff.controllers.marketcodes', 
											'ngff.controllers.channels', 
											'ngff.controllers.sobs', 
											'ngff.controllers.roomtypes', 
											'ngff.controllers.rooms', 
											'ngff.controllers.rawcompanys', 
											'ngff.controllers.rawpersons', 
											'ngff.controllers.bookings', 
											'ngff.controllers.paymenttypes', 
											'ngff.controllers.revenuetypes', 
											'ngff.controllers.revenuecodes', 
											'ngff.controllers.revenues', 
											'ngff.controllers.companys', 
											'ngff.controllers.persons', 
											'ngff.controllers.loyaltySchemes', 
											'ngff.controllers.loyaltyMembers', 
											'ngff.controllers.loyaltyPoints', 
											'ngff.controllers.loyaltyStatements', 
											'ngff.controllers.loyaltyTransactions' 

										]);
window.angular.module('ngff.services',
										[
											'ngff.services.global',
											'ngff.services.customers', 
											'ngff.services.sites', 
											'ngff.services.datasourcetypes', 
											'ngff.services.datasources', 
											'ngff.services.subscriptions', 
											'ngff.services.titles', 
											'ngff.services.countrys', 
											'ngff.services.currencys', 
											'ngff.services.bookingstatuss', 
											'ngff.services.bookingtypes', 
											'ngff.services.ratecodes', 
											'ngff.services.marketcodes', 
											'ngff.services.channels', 
											'ngff.services.sobs', 
											'ngff.services.roomtypes', 
											'ngff.services.rooms', 
											'ngff.services.rawcompanys', 
											'ngff.services.rawpersons', 
											'ngff.services.bookings', 
											'ngff.services.paymenttypes', 
											'ngff.services.revenuetypes', 
											'ngff.services.revenuecodes', 
											'ngff.services.revenues', 
											'ngff.services.companys', 
											'ngff.services.persons', 
											'ngff.services.loyaltySchemes', 
											'ngff.services.loyaltyMembers', 
											'ngff.services.loyaltyPoints', 
											'ngff.services.loyaltyStatements', 
											'ngff.services.loyaltyTransactions' 

										]);


var numLoadings = 0;
window.angular
	.module('loadingOnAJAX', [])
    .config(function ($httpProvider) {
        var loadingScreen = $('<div id="loadingscreen" style="position:fixed;top:0;left:0;right:0;bottom:0;z-index:10000;background-color:gray;background-color:rgba(70,70,70,0.0);"><img style="position:absolute;top:50%;left:50%;" alt="" src="data:image/gif;base64," /></div>')
            .appendTo($('body')).hide();
        $httpProvider.responseInterceptors.push('myHttpInterceptor');
        var spinnerFunction = function (data, headersGetter) {
            // todo start the spinner here
            //alert('start spinner');
            numLoadings++;
            loadingScreen.show();
            return data;
        };
        $httpProvider.defaults.transformRequest.push(spinnerFunction);
    })
// register the interceptor as a service, intercepts ALL angular ajax http calls
    .factory('myHttpInterceptor', function ($q, $window) {
        return function (promise) {
            return promise.then(function (response) {
                // do something on success
                // todo hide the spinner
                //alert('stop spinner');
            	if ( !(--numLoadings) ) 
            		$('#loadingscreen').hide(); 

                return response;

            }, function (response) {
                // do something on error
                // todo hide the spinner
                //alert('stop spinner');
            	if ( !(--numLoadings) ) 
            		$('#loadingscreen').hide(); 

                return $q.reject(response);
            });
        };
    });


=======
window.app = angular.module('loyalty3', ['ngCookies', 'ngResource', 'ui.bootstrap', 'ui.select2', 'ngTable', '$strap.directives', 'ngRoute', 'ngAnimate', 
										'ngff.controllers', 'ngff.directives', 'ngff.services', 'loadingOnAJAX']);

angular.module("template/pagination/pagination.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/pagination/pagination.html",
    "<ul class=\"pagination\">\n" +
    "  <li ng-repeat=\"page in pages\" ng-class=\"{active: page.active, disabled: page.disabled}\"><a ng-click=\"selectPage(page.number)\">{{page.text}}</a></li>\n" +
    "</ul>\n" +
    "");
}]);

// bundling dependencies
window.angular.module('ngff.controllers', 
										[
											'ngff.controllers.header', 
											'ngff.controllers.index', 
											'ngff.controllers.titles', 
											'ngff.controllers.currencys', 
											'ngff.controllers.countrys', 
											'ngff.controllers.subscriptions', 
											'ngff.controllers.customers', 
											'ngff.controllers.sites', 
											'ngff.controllers.companys', 
											'ngff.controllers.persons', 
											'ngff.controllers.rawcompanys', 
											'ngff.controllers.rawpersons', 
											'ngff.controllers.bookings', 
											'ngff.controllers.revenues', 
											'ngff.controllers.loyaltySchemes', 
											'ngff.controllers.loyaltyMembers', 
											'ngff.controllers.loyaltyPoints', 
											'ngff.controllers.loyaltyTransactions', 
											'ngff.controllers.loyaltyStatements'
										]);
window.angular.module('ngff.services',
										[
											'ngff.services.global',
											'ngff.services.titles',
											'ngff.services.currencys',
											'ngff.services.countrys',
											'ngff.services.subscriptions',
											'ngff.services.customers',
											'ngff.services.sites',
											'ngff.services.companys',
											'ngff.services.persons',
											'ngff.services.rawcompanys',
											'ngff.services.rawpersons',
											'ngff.services.bookings',
											'ngff.services.revenues',
											'ngff.services.loyaltySchemes',
											'ngff.services.loyaltyMembers',
											'ngff.services.loyaltyPoints',
											'ngff.services.loyaltyTransactions',
											'ngff.services.loyaltyStatements'
										]);


var numLoadings = 0;
window.angular
	.module('loadingOnAJAX', [])
    .config(function ($httpProvider) {
        var loadingScreen = $('<div id="loadingscreen" style="position:fixed;top:0;left:0;right:0;bottom:0;z-index:10000;background-color:gray;background-color:rgba(70,70,70,0.0);"><img style="position:absolute;top:50%;left:50%;" alt="" src="data:image/gif;base64," /></div>')
            .appendTo($('body')).hide();
        $httpProvider.responseInterceptors.push('myHttpInterceptor');
        var spinnerFunction = function (data, headersGetter) {
            // todo start the spinner here
            //alert('start spinner');
            numLoadings++;
            loadingScreen.show();
            return data;
        };
        $httpProvider.defaults.transformRequest.push(spinnerFunction);
    })
// register the interceptor as a service, intercepts ALL angular ajax http calls
    .factory('myHttpInterceptor', function ($q, $window) {
        return function (promise) {
            return promise.then(function (response) {
                // do something on success
                // todo hide the spinner
                //alert('stop spinner');
            	if ( !(--numLoadings) ) 
            		$('#loadingscreen').hide(); 

                return response;

            }, function (response) {
                // do something on error
                // todo hide the spinner
                //alert('stop spinner');
            	if ( !(--numLoadings) ) 
            		$('#loadingscreen').hide(); 

                return $q.reject(response);
            });
        };
    });


>>>>>>> 05644ebd4e842c71a618037d6cf2402b08f74c73
